"""
Configuration constants for the Computer Vision Evolution Demo V2.
All tunable parameters in one place for reproducibility.
"""

import os

# ----------------------------------------------------------------------
# Paths
# ----------------------------------------------------------------------
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
_MODEL_PATH = os.path.join(BASE_DIR, "..", "V1", "yolov8n.pt")
MODEL_PATH = _MODEL_PATH if os.path.isfile(_MODEL_PATH) else "yolov8n.pt"  # fallback: ultralytics will download
VIDEO_PATH = os.path.join(BASE_DIR, "..", "public", "sift.mp4")

# Set to 0 for webcam, or path string for video file
VIDEO_SOURCE = VIDEO_PATH

# ----------------------------------------------------------------------
# Resolution
# ----------------------------------------------------------------------
PANEL_W, PANEL_H = 540, 380          # Each panel in the 2x2 grid
PROCESS_W, PROCESS_H = 320, 240      # Downscaled for SIFT/YOLO inference
HEADER_H = 32                        # Panel header bar height
DASHBOARD_H = 130                    # Bottom dashboard height

# ----------------------------------------------------------------------
# Algorithm Parameters
# ----------------------------------------------------------------------
# Canny
CANNY_LOW = 30
CANNY_HIGH = 80

# SIFT
SIFT_N_FEATURES = 0                  # 0 = no limit, detect all features
SIFT_CONTRAST_THRESH = 0.03           # Lower = more sensitive
SIFT_MIN_SIZE = 0                     # No size filtering
SIFT_MIN_RESPONSE = 0.0              # No response filtering

# FLANN matcher for SIFT temporal matching
FLANN_INDEX_KDTREE = 1
FLANN_TREES = 5
FLANN_CHECKS = 50
LOWE_RATIO = 0.7                     # Lowe's ratio test threshold

# YOLO
YOLO_CONF_THRESH = 0.25

# ----------------------------------------------------------------------
# Perturbation Defaults
# ----------------------------------------------------------------------
NOISE_SIGMA_DEFAULT = 30
NOISE_SIGMA_STEP = 10
NOISE_SIGMA_MAX = 120

BLUR_KERNEL_DEFAULT = 11
BLUR_KERNEL_STEP = 4
BLUR_KERNEL_MAX = 51

ROTATION_ANGLE_DEFAULT = 15
ROTATION_ANGLE_STEP = 5
ROTATION_ANGLE_MAX = 90

BRIGHTNESS_DELTA_DEFAULT = -60
BRIGHTNESS_DELTA_STEP = 20
BRIGHTNESS_DELTA_MIN = -180
BRIGHTNESS_DELTA_MAX = 100

# ----------------------------------------------------------------------
# Metrics Smoothing (Exponential Moving Average)
# ----------------------------------------------------------------------
EMA_ALPHA = 0.3                       # Higher = more responsive, noisier

# ----------------------------------------------------------------------
# Colors (BGR for OpenCV)
# ----------------------------------------------------------------------
COLOR_BG = (30, 30, 30)
COLOR_HEADER_BG = (40, 40, 40)
COLOR_TEXT = (220, 220, 220)
COLOR_TEXT_DIM = (140, 140, 140)
COLOR_BORDER = (60, 60, 60)

# Per-algorithm accent colors
COLOR_ORIGINAL = (180, 180, 180)
COLOR_EDGE = (128, 255, 0)           # Green
COLOR_SIFT = (0, 180, 255)           # Orange
COLOR_YOLO = (255, 128, 0)           # Blue

# Perturbation indicator
COLOR_PERTURB = (0, 0, 255)          # Red

# Dashboard bars
COLOR_BAR_BG = (50, 50, 50)

# ----------------------------------------------------------------------
# Font
# ----------------------------------------------------------------------
FONT_FACE = 0                        # cv2.FONT_HERSHEY_SIMPLEX
FONT_SCALE_TITLE = 0.55
FONT_SCALE_METRIC = 0.42
FONT_SCALE_DASH = 0.40
FONT_SCALE_HELP = 0.35
FONT_THICKNESS = 1
